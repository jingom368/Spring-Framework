...
<head>
    ...
    <script>
	    window.onload = () => {
	    	...
	    	// 파일탐책창을 열어 선택한 파일 정보를 files에 할당
	    	...
	    	drag : 대상 객체를 드래그 하면서 마우스를 움직일 때 발생하는 이벤트
	    	dragenter : 마우스가 대상 객체의 위로 진입할 때 발생함.
	    	dragover : 드래그 하면서 마우스가 대상 객체의 영역 위에 자리 잡고 있을 때 발생
	    	drop : 드래그가 끝나서 드래그 하던 객체를 장소에 놓았을 때 발생함
	    	dragleave : 드래그가 끝나서 마우스가 대상 객체의 위에서 벗어날 때 발생함
	    	dragend : 대상객체를 드래그 하다가 마우스 버튼을 놓는 순간 발생함.
	    	*/
	    	
	    	// fileZone으로 dragenter 이벤트 발생 시 처리하는 이벤트 핸들러
	    	fileZone.addEventListener('dragenter', (e) => {
	    		e.stopPropagation(); // 상위 엘리먼트로의 이벤트 전파를 중단 
	    		e.preventDefault(); // 웹 브라우저의 고유 동작을 중단 
	    ...
	    var fileCount = 0; // 한 번 선택해서 가져오는 파일의 갯수
	    var fileNum = 0; // 파일 고유 넘버
	    var content_files = new Array();
	    var rowCount = 0;
	    
	    const fileCheck = (files) => {
	    	let filesArr = Object.values(files); // 객체에서 value에 해당되는 값을 뽑아서 배열로 변환
	    	...
	    	filesArr.forEach((file) => {
	    		...
	    		// FileReader 객체 : 웹애플리케이션이 비동기적으로 웹브라우저에서 파일을 읽을 때 사용하며,
	    		// <input type="file"> 태그를 이용하여 사용자가 선택한 파일들로부터 반환된 FileList 객체나
	    		// Drag & Drop 이벤트로 반환한 DataTransfer 객체로부터 데이터를 얻음
	    		var reader = new FileReader();
	    		
	    		reader.readAsDataURL(file); // 파일 읽기
	    		
	    		// reader.readAsDataURL(file) 실행으로 파일 읽기가 성공적으로 수행하고 난 후
	    		...
	    		reader.onload = (e) => {
	    			...
	    			// statusbar elembent내의 하위 element인 <div class='filename'>파일이름</div>를 생성
	    			let filename = "<div class='filename'>" + file.name + "</div>";
	    			
	    			// statusbar elembent내의 하위 element인 <div class='filesize'>파일이름</div>를 생성
	    			let sizeStr = "";
	    			...
	    			// statusbar element내의 하위 element인 삭제 버튼 생성
            ...
            await fetch(uploadURL, {
            	...
            	.then((data) => {
            		if(data.message === 'GOOD') {
            			alert("게시물이 수정되었습니다.");
            			document.location.href='/board/view?seqno=' 
            					+ seqno.value + '&page=' + page.value + '&keyword=' + keyword.value;
            			console.log('seqno.value : ' + seqno.value)
...
%>
    ...
    <div class = "main">
        ...
        <br>
        <div id="formZone">
            <form class="ModifyForm" name="ModifyForm" method="post" enctype="x-www-urlencoded">
                <input type="text" id="writer" class="input_field" name="writer" value="${view.writer}" disabled>
                ...
                <br><br>
                <textarea id="content" class="input_content" cols="100" rows="500" name="content">${view.content}</textarea>
